...
void transform(ST_Data *FLASH_DATA){
	...
	if(FLASH_DATA->DATA_8_LEN >= FLASH_PAGE_SIZE){				
		for(int i = 0; i < FLASH_PAGE_SIZE/4 ; i++){	
			...
			INDEX = FLASH_DATA->DATA_8_INDEX_HEAD+3;			
			if(INDEX >=MAX_round_queue) INDEX -= MAX_round_queue;						
			FLASH_DATA->DATA_32[FLASH_DATA->DATA_32_INDEX] += FLASH_DATA->DATA_8[INDEX];  //INDEX+3
			FLASH_DATA->DATA_32[FLASH_DATA->DATA_32_INDEX] <<= 8;
			
			INDEX = FLASH_DATA->DATA_8_INDEX_HEAD+2;
			if(INDEX >=MAX_round_queue) INDEX -= MAX_round_queue;		
			FLASH_DATA->DATA_32[FLASH_DATA->DATA_32_INDEX] += FLASH_DATA->DATA_8[INDEX];  //INDEX+2
			FLASH_DATA->DATA_32[FLASH_DATA->DATA_32_INDEX] <<= 8;

			INDEX = FLASH_DATA->DATA_8_INDEX_HEAD+1;
			if(INDEX >=MAX_round_queue) INDEX -= MAX_round_queue;				
			FLASH_DATA->DATA_32[FLASH_DATA->DATA_32_INDEX] += FLASH_DATA->DATA_8[INDEX]; //INDEX+1
			FLASH_DATA->DATA_32[FLASH_DATA->DATA_32_INDEX] <<= 8;
			
			INDEX = FLASH_DATA->DATA_8_INDEX_HEAD;
			if(INDEX >=MAX_round_queue) INDEX -= MAX_round_queue;	
			FLASH_DATA->DATA_32[FLASH_DATA->DATA_32_INDEX] += FLASH_DATA->DATA_8[INDEX];	//INDEX
